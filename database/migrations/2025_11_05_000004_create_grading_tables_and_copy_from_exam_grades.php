    ...
    public function up(): void
    {
        if (!Schema::hasTable('grading_schemes')) {
            Schema::create('grading_schemes', function (Blueprint $t) {
                $t->id();
                $t->string('name');
                $t->string('type')->default('percentage'); // CBC/percentage/GPA
        ...
        if (!Schema::hasTable('grading_bands')) {
            Schema::create('grading_bands', function (Blueprint $t) {
                ...
                $t->unsignedDecimal('min', 6, 2);
                $t->unsignedDecimal('max', 6, 2);
                $t->string('label');       // A, EE, etc. (your model uses 'label')
                $t->string('descriptor')->nullable(); // long text
                $t->unsignedDecimal('rank', 5, 2)->default(0); // points or rank
                $t->timestamps();
            });
        }
        ...
        // Copy current exam_grades to a scheme per exam_type (idempotent-ish)
                ...
                foreach ($types as $type) {
                    ...
                    foreach ($rows as $r) {
                        DB::table('grading_bands')->insert([
                            ...
                            'min' => $r->percent_from,
                            'max' => $r->percent_upto,
                            'label' => $r->grade_name,             // short grade
