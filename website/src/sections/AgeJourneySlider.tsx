    ...
    <section className="bg-gradient-to-b from-[#faf7ff] to-white py-20">
      <div className="mx-auto max-w-6xl px-4 lg:px-8">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center">
          <p className="text-sm uppercase tracking-widest text-[#5B2C8E]">Signature Experience</p>
          <h2 className="mt-2 font-serif text-3xl font-bold text-[#2a1145] md:text-4xl">The Age Journey: 3 to 15</h2>
        ...
        <div className="mt-12">
          <input
            ...
            className="h-2 w-full cursor-pointer appearance-none rounded-full bg-[#e8dff5] accent-[#5B2C8E]"
            aria-label="Age journey slider"
          />
          <div className="mt-2 flex justify-between text-xs text-[#5B2C8E]">
            <span>Age 3</span>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={step.age} ref={cardRef} className="mt-10 grid gap-6 rounded-3xl border border-[#e8dff5] bg-white p-8 shadow-xl md:grid-cols-2">
            <div>
              ...
              <h3 className="mt-2 font-serif text-3xl font-bold text-[#2a1145]">{step.classroom}</h3>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <h4 className="font-semibold text-[#5B2C8E]">Activities</h4>
                <ul className="mt-2 space-y-1 text-sm text-[#4a3a5c]">{step.activities.map((a) => <li key={a}>• {a}</li>)}</ul>
              </div>
              <div>
                <h4 className="font-semibold text-[#5B2C8E]">Milestones</h4>
