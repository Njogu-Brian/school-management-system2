  ...
  return (
    <section className="py-20">
      <div className="mx-auto max-w-6xl px-4 lg:px-8">
        <motion.h2 initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="text-center font-serif text-3xl font-bold text-[var(--rk-purple-deep)]">
          Why Royal Kings
        </motion.h2>
        ...
        <div className="mt-12 flex flex-wrap items-center justify-center gap-3">
          {LEARNING_PATHWAY.map((stage, i) => (
            <div key={stage} className="flex items-center gap-3">
              <span className="rounded-full bg-[#D4AF37] px-4 py-2 text-sm font-semibold text-[var(--rk-purple-deep)]">{stage}</span>
...
export function ProgramsGrid() {
  return (
    <section className="py-20">
      <div className="mx-auto max-w-6xl px-4 lg:px-8">
        <h2 className="text-center font-serif text-3xl font-bold text-[var(--rk-purple-deep)]">Programs & Co-Curricular</h2>
        <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-7">
          {PROGRAMS.map((p) => (
            <div key={p.name} className="flex flex-col items-center rounded-2xl bg-[#faf7ff] p-4 text-center transition hover:bg-[#f0e8ff]">
              ...
              <span className="mt-2 text-sm font-semibold text-[var(--rk-purple)]">{p.name}</span>
            </div>
          ))}
        </div>
      </div>
