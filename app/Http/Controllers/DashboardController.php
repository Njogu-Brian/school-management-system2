...
class DashboardController extends Controller
{
    ...
    private function buildDashboardData(Request $request, string $role = 'admin'): array
    {
        ...
        $cacheKey = $this->dashboardCacheKey($role, $filters);

        return Cache::remember(
            $cacheKey,
            now()->addMinutes(5),
            function () use ($filters, $role) {
    ...
    protected function dashboardCacheKey(string $role, array $filters): string
    {
        ksort($filters);

        return 'dashboard:'.$role.':'.md5(json_encode($filters));
    }
