...
function Assert-UsersVersion {
  ...
  $code = [int]$Matches[1]
  Write-Host "Users native versionCode = $code (Play last used 14; need >= 15)"
  if ($code -lt 15) { throw "Users versionCode $code would conflict with Play Console" }
}
...
function Assert-AdminVersion {
  ...
  $code = [int]$Matches[1]
  Write-Host "Admin app.config versionCode = $code (Play last used 16; need >= 17)"
  if ($code -lt 17) { throw "Admin versionCode $code would conflict with Play Console" }
}
