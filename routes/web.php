    ...
    */
    Route::prefix('transport')->name('transport.')
        ...
        ->group(function () {
            ...
            Route::post('trips/{trip}/assign/points', [TripController::class, 'assignUpdatePoints'])->name('trips.assign.points');
            Route::post('trips/{trip}/assign/duplicate-leg', [TripController::class, 'assignDuplicateLeg'])->name('trips.assign.duplicate-leg');
            ...
            Route::post('dropoffpoints/resolve', [DropOffPointController::class, 'resolve'])->name('dropoffpoints.resolve');
    ...
    Route::prefix('hr')->name('hr.')
        ...
        ->group(function () {
            ...
            Route::prefix('payroll')->name('payroll.')->group(function () {
                ...
                Route::post('/periods/{id}/approve', [\App\Http\Controllers\Hr\PayrollPeriodController::class, 'approve'])->name('periods.approve');
...
Route::get('/families/{family}/update-link', [FamilyUpdateController::class, 'showLink'])
    ...
    */
    Route::prefix('communication')->middleware('role:Super Admin|Admin|Secretary')->group(function () {
        ...
        Route::get('sms-dlr', [CommunicationController::class, 'smsDlrUpload'])->name('communication.sms-dlr');
        Route::post('sms-dlr', [CommunicationController::class, 'smsDlrProcess'])->name('communication.sms-dlr.process');

        // Logs & Conversations (track bulk campaigns and delivery status)
    ...
    */
    ...
    Route::prefix('finance')->name('finance.')->middleware('role:Super Admin|Admin|Secretary|Accountant|Finance Officer|Director')->group(function () {
        ...
        Route::prefix('optional-fees')->name('optional_fees.')->group(function () {
            ...
            Route::post('/duplicate/preview', [OptionalFeeController::class, 'duplicatePreview'])->name('duplicate.preview');
            Route::post('/duplicate/commit',  [OptionalFeeController::class, 'duplicateCommit'])->name('duplicate.commit');
        });
        ...
        Route::post('payments/bulk-allocate-unallocated', [PaymentController::class, 'bulkAllocateUnallocated'])->middleware('throttle:10,1')->name('payments.bulk-allocate-unallocated');
        Route::post('payments/bulk-send-preview', [PaymentController::class, 'bulkSendPreview'])->middleware('throttle:10,1')->name('payments.bulk-send-preview');
        Route::post('payments/bulk-send', [PaymentController::class, 'bulkSend'])->middleware('throttle:5,1')->name('payments.bulk-send');
        ...
        Route::prefix('bank-statements')->name('bank-statements.')->group(function () {
            ...
            Route::post('/bulk-confirm', [\App\Http\Controllers\Finance\BankStatementController::class, 'bulkConfirm'])->middleware('throttle:10,1')->name('bulk-confirm');
            Route::post('/bulk-confirm-and-create', [\App\Http\Controllers\Finance\BankStatementController::class, 'bulkConfirmAndCreatePayments'])->middleware('throttle:10,1')->name('bulk-confirm-and-create');
            Route::post('/bulk-archive', [\App\Http\Controllers\Finance\BankStatementController::class, 'bulkArchive'])->middleware('throttle:10,1')->name('bulk-archive');
            Route::post('/auto-assign', [\App\Http\Controllers\Finance\BankStatementController::class, 'autoAssign'])->middleware('throttle:5,1')->name('auto-assign');
            ...
            Route::post('/reconcile-payments', [\App\Http\Controllers\Finance\BankStatementController::class, 'reconcilePayments'])->name('reconcile-payments');
        ...
        Route::delete('balance-brought-forward/{student}', [\App\Http\Controllers\Finance\BalanceBroughtForwardController::class, 'destroy'])->name('balance-brought-forward.destroy');
        ...
        // Fee Reminders (explicit routes before resource to avoid capture)
    ...
    */
    ...
    Route::prefix('admin/alerts')->name('admin.alerts.')->group(function () {
        ...
        Route::get('/sms-balance', [\App\Http\Controllers\AdminAlertController::class, 'smsBalance'])
