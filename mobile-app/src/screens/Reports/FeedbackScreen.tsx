...
export const FeedbackScreen = () => {
    ...
    const load = async () => {
        try {
            setError(null);
            setLoading(true);
            const data = await feedbackApi.getTemplate();
            setTpl(data);
        } catch (e: any) {
    ...
    const submit = async () => {
        ...
        setSubmitting(true);
        try {
            const submission = await feedbackApi.submit({
            ...
            for (const q of questions) {
                ...
                const f = files[q.id];
                if (!f) continue;
                await feedbackApi.uploadFileAnswer({
            ...
            Alert.alert('Submit failed', e?.message ?? 'Could not send feedback.');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <SafeAreaView style={[layoutStyles.flex1, { backgroundColor: isDark ? colors.backgroundDark : colors.backgroundLight }]}>
            ...
            <ScrollView contentContainerStyle={{ padding: SPACING.xl, paddingTop: SPACING.md }}>
                ...
                {tpl?.description ? (
                    <Text style={[styles.desc, { color: isDark ? colors.textSubDark : colors.textSubLight }]}>{tpl.description}</Text>
                ) : null}

                <View style={[styles.block, { backgroundColor: isDark ? colors.surfaceDark : colors.surfaceLight, borderColor: isDark ? colors.borderDark : colors.borderLight }]}>
                    ...
                    <Text style={[styles.help, { color: isDark ? colors.textSubDark : colors.textSubLight }]}>
                        If enabled, your name will not be attached to this feedback.
                    </Text>
                </View>

                {loading ? (
                    <Text style={{ color: isDark ? colors.textSubDark : colors.textSubLight, marginTop: SPACING.lg }}>Loading…</Text>
                ) : null}
                        ...
                        if (q.type === 'short_text' || q.type === 'long_text') {
                            return (
                                <View key={q.id} style={[styles.block, { backgroundColor: surface, borderColor: border }]}>
                                    ...
                                    {q.help_text ? (
                                        <Text style={[styles.help, { color: isDark ? colors.textSubDark : colors.textSubLight }]}>{q.help_text}</Text>
                                    ) : null}
                                    <TextInput
                        ...
                        if (q.type === 'file_upload') {
                            ...
                            return (
                                <View key={q.id} style={[styles.block, { backgroundColor: surface, borderColor: border }]}>
                                    ...
                                    <TouchableOpacity style={[styles.fileBtn, { borderColor: border }]} onPress={() => pickFile(q.id)}>
                                        <Text style={{ color: colors.primary, fontWeight: '700' }}>{f ? 'Change file' : 'Pick a file'}</Text>
                                    </TouchableOpacity>
                                    {f ? (
                                        <Text style={[styles.help, { color: isDark ? colors.textSubDark : colors.textSubLight }]}>
                                            Selected: {f.name}
                ...
                <View style={{ marginTop: SPACING.lg }}>
                    <Button title={submitting ? 'Sending…' : 'Send feedback'} onPress={submit} disabled={submitting || loading} />
                </View>
            </ScrollView>
        </SafeAreaView>
    );
