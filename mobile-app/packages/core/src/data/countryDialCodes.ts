...
export const DEFAULT_COUNTRY_DIAL_CODE = '+254';
...
export const COUNTRY_DIAL_CODES: CountryDialCode[] = (() => {
  ...
  const kenya = mapped.filter((row) => row.name === 'Kenya');
  const others = mapped
    .filter((row) => row.name !== 'Kenya')
    .sort((a, b) => a.name.localeCompare(b.name));
  return [...kenya, ...others];
})();

export function countryDialLabel(code: string | null | undefined): string {
  const match = COUNTRY_DIAL_CODES.find((row) => row.code === (code || DEFAULT_COUNTRY_DIAL_CODE));
  return match?.label ?? `${code || DEFAULT_COUNTRY_DIAL_CODE}`;
}
