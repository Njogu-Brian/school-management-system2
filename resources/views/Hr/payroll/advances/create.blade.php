...
<div class="container-fluid">
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h2 class="mb-0">New Staff Advance</h2>
      <small class="text-muted">Create a new advance loan for staff</small>
    </div>
  ...
  <div class="card shadow-sm">
    <div class="card-body">
      <form action="{{ route('hr.payroll.advances.store') }}" method="POST">
        @csrf
        ...
        <div class="d-flex justify-content-end gap-2">
          <a href="{{ route('hr.payroll.advances.index') }}" class="btn btn-outline-secondary">Cancel</a>
...
document.getElementById('repayment_method').addEventListener('change', function() {
  ...
  if (method === 'installments') {
    installmentField.querySelector('input').required = true;
  } else {
    installmentField.querySelector('input').required = false;
  }
  
  if (method === 'monthly_deduction') {
    monthlyField.querySelector('input').required = true;
  } else {
    monthlyField.querySelector('input').required = false;
  }
});
