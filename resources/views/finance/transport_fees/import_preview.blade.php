            ...
            <table class="finance-table align-middle">
              <thead>
                <tr>
                  ...
                  @if($needsMatchingCount > 0 || $needsConfirmationCount > 0)
              ...
              <tbody>
                @foreach($preview as $index => $row)
                  @php 
                    ...
                    $showRow = $changeType !== 'existing' || $needsMatching || $needsConfirmation;
                  @endphp
                  @if($showRow)
                  <tr class="{{ $isOk && !$needsMatching && !$needsConfirmation ? '' : ($needsConfirmation ? 'table-info' : 'table-warning') }}">
                    <td>
                    ...
                    <td>
                      @if($isOk && !$needsMatching && !$needsConfirmation)
