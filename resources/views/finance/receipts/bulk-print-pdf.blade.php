...
@php
    ...
    $allocations = $receiptData['allocations'] ?? collect();
    $school = $receiptData['school'] ?? [];
    $branding = $receiptData['branding'] ?? [];
    $receipt_number = $receiptData['receipt_number'] ?? $payment->receipt_number;
    $date = $receiptData['date'] ?? ($payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('d M Y') : date('d M Y'));
    $total_amount = $receiptData['total_amount'] ?? $payment->amount;
    $total_balance_before = $receiptData['total_balance_before'] ?? 0;
    $total_balance_after = $receiptData['total_balance_after'] ?? 0;
    $total_outstanding_balance = $receiptData['total_outstanding_balance'] ?? 0;
    $total_invoices = $receiptData['total_invoices'] ?? 0;
    $payment_method = $receiptData['payment_method'] ?? ($payment->paymentMethod->name ?? $payment->payment_method ?? 'Cash');
    $transaction_code = $receiptData['transaction_code'] ?? $payment->transaction_code;
    $narration = $receiptData['narration'] ?? $payment->narration;
    ...
    <div class="receipt-container" style="@if($index > 0) page-break-before: always !important; @endif">
        ...
        <table class="receipt-details-table">
            ...
            <tr>
                ...
                <td class="detail-value" style="text-align: right; width: 25%;">{{ $date }}</td>
            </tr>
            @else
            <tr>
                <td colspan="2"></td>
        ...
        <div class="total-section">
            @php
                ...
                $hasTotalOutstanding = $total_outstanding_balance > 0;
            @endphp
            
            <div class="total-row">
                <span>Total Invoices:</span>
                <span><strong>Ksh {{ number_format($total_invoices, 2) }}</strong></span>
            </div>
            <div class="total-row">
            ...
            <div class="payment-info">
                Payment Method: {{ $payment_method }}@if($transaction_code), Reference: {{ $transaction_code }}@endif
            </div>
            ...
            <div class="total-row grand-total" style="border-top: 1px solid #3a1a59; padding-top: 6px; margin-top: 6px; color: #dc3545;">
                ...
                <span style="font-size: 12px; font-weight: bold;">Ksh {{ number_format($total_outstanding_balance, 2) }}</span>
            </div>
            @else
