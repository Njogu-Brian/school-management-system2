            ...
            <div class="finance-card finance-animate receipt-card">
                <div class="finance-card-body p-4">
                    @include('finance.receipts.pdf.template', [
                        ...
                        'branding' => $branding ?? [],
                        'receipt_number' => $receipt_number ?? $displayReceiptNumber,
                        'date' => $date ?? ($payment->receipt_date ? \Carbon\Carbon::parse($payment->receipt_date)->format('d/m/Y') : ($payment->payment_date ? \Carbon\Carbon::parse($payment->payment_date)->format('d/m/Y') : date('d/m/Y'))),
                        'student' => $student ?? null,
                        'allocations' => $allocations ?? collect(),
                        'total_amount' => $total_amount ?? $payment->amount,
                        'total_balance_before' => $total_balance_before ?? ($totalBalanceBefore ?? 0),
                        'total_balance_after' => $total_balance_after ?? ($totalBalanceAfter ?? 0),
                        ...
                        'narration' => $narration ?? $payment->narration,
                        'receipt_header' => $receipt_header ?? null,
