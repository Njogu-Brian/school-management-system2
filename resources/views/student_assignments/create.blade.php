    ...
    <div class="settings-shell">
        <div class="page-header">
            <div>
                ...
                <p class="text-muted mb-0">Assign trips to a student. Drop-off points are set during transport fee import.</p>
            </div>
                ...
                <form action="{{ route('transport.student-assignments.store') }}" method="POST" class="row g-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="morning_trip_id" id="morning_trip_id" class="form-select">
                            ...
                            @foreach ($trips as $trip)
                                <option value="{{ $trip->id }}" {{ old('morning_trip_id') == $trip->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12">
                        ...
                        <select name="evening_trip_id" id="evening_trip_id" class="form-select">
                            ...
                            @foreach ($trips as $trip)
                                <option value="{{ $trip->id }}" {{ old('evening_trip_id') == $trip->id ? 'selected' : '' }}>
                                    {{ optional($trip->vehicle)->vehicle_number ?? 'N/A' }} - {{ $trip->name }}
                                </option>
                            @endforeach
                        </select>
