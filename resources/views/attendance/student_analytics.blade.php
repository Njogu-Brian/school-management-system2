...
<div class="container-fluid">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h2 class="mb-0">Attendance Analytics: {{ $student->full_name }}</h2>
      <small class="text-muted">Admission #{{ $student->admission_number }}</small>
    </div>
    ...
    <div class="card-body">
      <form method="GET" class="row g-3">
        ...
        <div class="col-md-3 d-flex align-items-end">
          <a href="{{ route('attendance.student-analytics', $student) }}" class="btn btn-outline-secondary w-100">
            ...
            @foreach($trends as $trend)
              <tr>
                ...
                <td class="text-center">
                  <span class="badge bg-success">{{ $trend['present'] }}</span>
                </td>
                <td class="text-center">
                  <span class="badge bg-danger">{{ $trend['absent'] }}</span>
                </td>
                <td class="text-center">
                  <span class="badge bg-warning">{{ $trend['late'] }}</span>
                </td>
                <td class="text-center">
                  <span class="badge bg-{{ $trend['percentage'] < 75 ? 'danger' : ($trend['percentage'] < 90 ? 'warning' : 'success') }}">
          ...
          <tbody>
            @foreach($subjectStats as $stat)
              <tr>
                ...
                <td class="text-center">
                  <span class="badge bg-{{ $stat['percentage'] < 75 ? 'danger' : ($stat['percentage'] < 90 ? 'warning' : 'success') }}">
