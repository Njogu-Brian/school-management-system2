                ...
                <form action="{{ route('hr.payroll.custom-deductions.update', $deduction->id) }}" method="POST" class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="frequency" id="frequency" class="form-select @error('frequency') is-invalid @enderror" required>
                            ...
                            <option value="quarterly" @selected(old('frequency', $deduction->frequency)==='quarterly')>Quarterly (Jan/Apr/Jul/Oct)</option>
                            <option value="yearly" @selected(old('frequency', $deduction->frequency)==='yearly')>Yearly (January)</option>
                            <option value="custom_months" @selected(old('frequency', $deduction->frequency)==='custom_months')>Selected months</option>
                        </select>
                        @error('frequency')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    @php
                        $selectedMonths = old('applicable_months', $deduction->applicable_months ?? []);
                    @endphp
                    <div class="col-12 {{ old('frequency', $deduction->frequency) === 'custom_months' ? '' : 'd-none' }}" id="applicable_months_field">
                            ...
                            @foreach(\App\Models\CustomDeduction::monthLabels() as $num => $label)
                                <label class="form-check form-check-inline border rounded px-2 py-1 mb-0">
                                    <input type="checkbox" class="form-check-input" name="applicable_months[]" value="{{ $num }}"
                                        @checked(in_array($num, $selectedMonths) || in_array((string) $num, $selectedMonths))>
