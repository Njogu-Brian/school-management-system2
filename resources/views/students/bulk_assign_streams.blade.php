...
<div class="container-fluid">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h2 class="mb-0">Bulk Assign Students to Streams</h2>
      <small class="text-muted">Select a classroom and assign multiple students to a stream at once</small>
    </div>
  ...
  @if($selectedClassroom)
    {{-- Step 2: Select Students and Stream --}}
    <div class="card">
      <div class="card-header">
        <h5 class="mb-0">
          <i class="bi bi-2-circle"></i> Step 2: Select Students and Assign Stream
          <span class="badge bg-primary ms-2">{{ $selectedClassroom->name }}</span>
        </h5>
      </div>
      <div class="card-body">
        @if($students->count() > 0)
                    ...
                    <tr>
                      ...
                      <td>
                        <small class="text-muted">
                          {{ $student->parent?->primary_contact_phone ?? $student->parent?->primary_contact_email ?? 'No contact' }}
                        </small>
                      </td>
                    </tr>
                  @endforeach
...
<script>
  ...
  function updateSelectedCount() {
    ...
    document.getElementById('submitBtn').disabled = checked === 0 || !document.querySelector('select[name="stream_id"]').value;
  }
  ...
  // Update submit button when stream is selected
  document.querySelector('select[name="stream_id"]').addEventListener('change', updateSelectedCount);

  // Form validation
  document.getElementById('bulkStreamForm').addEventListener('submit', function(e) {
    const checked = document.querySelectorAll('.student-checkbox:checked').length;
    const streamId = document.querySelector('select[name="stream_id"]').value;
    ...
    return confirm(`Are you sure you want to assign ${checked} student(s) to the selected stream?`);
  });
</script>
@endpush
@endsection
