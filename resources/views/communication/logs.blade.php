                        ...
                        <tbody>
                            @forelse($logs as $log)
                                @php
                                    ...
                                    $cls = match($status) {
                                        'delivered','sent','success' => 'pill-badge',
                                        'pending','queued'           => 'pill-badge',
                                        'blacklisted','failed','undelivered','rejected' => 'pill-badge',
                                        default => 'pill-badge'
                                    };
                                    $safeTitle = $log->title ?: \Illuminate\Support\Str::limit(strip_tags($log->message), 40, '…');
                                    $sentAt = $log->sent_at ? \Illuminate\Support\Carbon::parse($log->sent_at)->format('Y-m-d H:i') : '-';
                                @endphp
                                <tr>
                                    <td class="text-nowrap"><strong>{{ $safeTitle }}</strong></td>
                                    <td><span class="input-chip">{{ ucfirst($log->recipient_type ?? '-') }}</span></td>
                                    <td><span class="pill-badge">{{ strtoupper($log->channel) }}</span></td>
                                    <td><span class="pill-badge">{{ ucfirst($status) }}</span></td>
                                            ...
                                            <div class="modal-body">
                                                <dl class="row">
                                                    ...
                                                    @if(!empty($log->delivered_at))
                                                        ...
                                                        <dd class="col-sm-9">{{ \Illuminate\Support\Carbon::parse($log->delivered_at)->format('Y-m-d H:i') }}</dd>
                                                    @endif
