  ...
  <div class="settings-shell">
    <div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-3">
      <div>
        ...
        <h1 class="mb-1">Add New Stream</h1>
        <p class="text-muted mb-0">Create a stream and link it to classrooms.</p>
      </div>
    ...
    <form action="{{ route('academics.streams.store') }}" method="POST" class="settings-card">
              ...
              @foreach ($classrooms as $classroom)
                <div class="col-md-4">
                  <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="classroom_ids[]" value="{{ $classroom->id }}" id="classroom_{{ $classroom->id }}" @if(old('classroom_ids') && in_array($classroom->id, old('classroom_ids'))) checked @endif>
      ...
      <div class="card-footer d-flex justify-content-end gap-2">
        ...
        <button type="submit" class="btn btn-settings-primary">Create Stream</button>
      </div>
    </form>
  </div>
</div>
