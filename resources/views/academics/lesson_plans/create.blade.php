  ...
  <div class="settings-shell">
    <div class="page-header d-flex justify-content-between align-items-start flex-wrap gap-3">
      <div>
        ...
        <h1 class="mb-1">Create Lesson Plan</h1>
        <p class="text-muted mb-0">Capture objectives, outcomes, and activities.</p>
      </div>
        ...
        <form action="{{ route('academics.lesson-plans.store') }}" method="POST">
            ...
            <div class="col-md-6">
              ...
              <select name="scheme_of_work_id" class="form-select">
                ...
                @foreach($schemes as $scheme)
                  <option value="{{ $scheme->id }}" {{ old('scheme_of_work_id') == $scheme->id ? 'selected' : '' }}>{{ $scheme->title }} - {{ $scheme->classroom->name }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-6">
          ...
          <div class="row g-3 mt-1">
            ...
            <div class="col-md-3">
              ...
              <input type="date" name="planned_date" class="form-control @error('planned_date') is-invalid @enderror" value="{{ old('planned_date') }}" required>
          ...
          <div class="mt-3">
            ...
            <select name="substrand_id" class="form-select">
              ...
              @foreach($substrands as $substrand)
                <option value="{{ $substrand->id }}" {{ old('substrand_id') == $substrand->id ? 'selected' : '' }}>{{ $substrand->strand->name }} - {{ $substrand->name }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="mt-3">
            ...
            <textarea name="learning_objectives" class="form-control" rows="3" placeholder="One objective per line">{{ old('learning_objectives') }}</textarea>
          ...
          <div class="mt-3"><label class="form-label">Introduction</label><textarea name="introduction" class="form-control" rows="3">{{ old('introduction') }}</textarea></div>
          <div class="mt-3"><label class="form-label">Lesson Development</label><textarea name="lesson_development" class="form-control" rows="5">{{ old('lesson_development') }}</textarea></div>
          <div class="mt-3"><label class="form-label">Assessment</label><textarea name="assessment" class="form-control" rows="3">{{ old('assessment') }}</textarea></div>
          <div class="mt-3"><label class="form-label">Conclusion</label><textarea name="conclusion" class="form-control" rows="2">{{ old('conclusion') }}</textarea></div>

          <div class="d-flex justify-content-end gap-2 mt-3">
            <a href="{{ route('academics.lesson-plans.index') }}" class="btn btn-ghost-strong">Cancel</a>
            <button type="submit" class="btn btn-settings-primary">Create Lesson Plan</button>
          </div>
        </form>
      </div>
    </div>
