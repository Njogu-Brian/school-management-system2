...
<div class="settings-page">
  <div class="settings-shell">
    ...
    <div class="settings-card border-primary-subtle mb-3">
      <div class="card-header d-flex align-items-center gap-2"><i class="bi bi-magic"></i><h5 class="mb-0">Auto-Generate from CBC Curriculum</h5></div>
      <div class="card-body">
        <form action="{{ route('academics.schemes-of-work.generate') }}" method="POST" id="autoGenerateForm" class="row g-3">
          @csrf
          <div class="col-md-6">
          ...
          <div class="col-md-4">
            ...
            <small class="text-muted">Auto-calculates from substrands if empty.</small>
          </div>
          <div class="col-md-4">
          ...
          <div class="col-12">
            <div class="form-check form-switch">
              ...
              <label class="form-check-label" for="generate_lesson_plans">Also generate lesson plans</label>
            </div>
          </div>

          <div class="row g-3" id="lesson_plans_options" style="display: none;">
            <div class="col-md-6">
          ...
          <div class="col-12 d-flex justify-content-end gap-2">
            <button type="submit" class="btn btn-settings-primary"><i class="bi bi-magic"></i> Auto-Generate Scheme of Work</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div class="settings-card">
      <div class="card-header d-flex align-items-center gap-2"><i class="bi bi-pencil-square"></i><h5 class="mb-0">Manual Create</h5></div>
      <div class="card-body">
        <form action="{{ route('academics.schemes-of-work.store') }}" method="POST" class="row g-3">
          @csrf
          <div class="col-md-6">
            <label class="form-label">Subject <span class="text-danger">*</span></label>
          ...
          <div class="col-12">
            ...
            <small class="text-muted">Hold Ctrl/Cmd to select multiple. Strands load after selecting subject and classroom.</small>
          </div>
          ...
          <div class="col-12 d-flex justify-content-end gap-2">
            <a href="{{ route('academics.schemes-of-work.index') }}" class="btn btn-ghost-strong">Cancel</a>
            <button type="submit" class="btn btn-settings-primary">Create Scheme</button>
          </div>
        </form>
      </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadStrands(){
    const subjectId=subjectSelect.value; const classroomId=classroomSelect.value;
    if(!subjectId||!classroomId){ strandsSelect.innerHTML='<option value="">Select subject and classroom first</option>'; return; }
    fetch(`{{ route('academics.schemes-of-work.get-strands') }}?subject_id=${subjectId}&classroom_id=${classroomId}`)
      .then(r=>r.json())
      .then(data=>{
        strandsSelect.innerHTML='';
        if(data.length===0){ strandsSelect.innerHTML='<option value="">No strands found for this subject and classroom</option>'; }
        else {
          data.forEach(strand=>{ const option=document.createElement('option'); option.value=strand.id; option.textContent=`${strand.name} (${strand.code})`; strandsSelect.appendChild(option); });
        }
      })
      .catch(()=>{ strandsSelect.innerHTML='<option value="">Error loading strands</option>'; });
  }
  ...
  function loadAutoStrands(){
    const subjectId=autoSubjectSelect.value; const classroomId=autoClassroomSelect.value;
    if(!subjectId||!classroomId){ autoStrandsContainer.innerHTML='<p class="text-muted mb-0">Select subject and classroom to load strands</p>'; return; }
    fetch(`{{ route('academics.schemes-of-work.get-strands') }}?subject_id=${subjectId}&classroom_id=${classroomId}`)
      .then(r=>r.json())
      .then(data=>{
        if(data.length===0){ autoStrandsContainer.innerHTML='<p class="text-muted mb-0">No strands found for this subject and classroom</p>'; }
        else {
          let html='';
          data.forEach(strand=>{ html+=`<div class="form-check"><input class="form-check-input" type="checkbox" name="strand_ids[]" value="${strand.id}" id="auto_strand_${strand.id}"><label class="form-check-label" for="auto_strand_${strand.id}">${strand.name} (${strand.code})</label></div>`; });
          autoStrandsContainer.innerHTML=html;
        }
      })
      .catch(()=>{ autoStrandsContainer.innerHTML='<p class="text-danger mb-0">Error loading strands</p>'; });
  }

  autoSubjectSelect.addEventListener('change', loadAutoStrands);
  autoClassroomSelect.addEventListener('change', loadAutoStrands);
  generateLessonPlansCheck.addEventListener('change', function(){ lessonPlansOptions.style.display=this.checked ? 'block' : 'none'; });
});
</script>
@endpush
@endsection
