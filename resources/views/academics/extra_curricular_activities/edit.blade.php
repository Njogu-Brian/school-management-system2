        ...
        <div class="card-body">
            <form action="{{ route('academics.extra-curricular-activities.update', $extra_curricular_activity) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="fee_amount" class="form-control @error('fee_amount') is-invalid @enderror" 
                               step="0.01" min="0" value="{{ old('fee_amount', $extra_curricular_activity->fee_amount) }}" 
                               placeholder="0.00">
                        <small class="text-muted">Leave empty if activity is free. Setting a fee will automatically create/update a votehead and add it to class fee structures.</small>
                        @if($extra_curricular_activity->votehead)
                            <div class="alert alert-info mt-2">
                                <small><i class="bi bi-info-circle"></i> Linked to votehead: <strong>{{ $extra_curricular_activity->votehead->name }}</strong></small>
                            </div>
                        @endif
                        @error('fee_amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        <div class="form-check mt-4">
                            <input class="form-check-input" type="checkbox" name="auto_invoice" value="1" id="auto_invoice" 
                                   {{ old('auto_invoice', $extra_curricular_activity->auto_invoice) ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <small class="text-muted">Hold Ctrl/Cmd to select multiple students. Use search to filter. Currently assigned: {{ count($selectedStudents) }} student(s)</small>
                </div>
                <script>
